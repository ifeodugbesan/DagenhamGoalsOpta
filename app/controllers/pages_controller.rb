class PagesController < ApplicationController
  ...
  def stats
    ...
    season_three_matches = User.where.not(name: 'OG Home').where.not(name: 'OG Away').where.not(name: 'No Assist').sort_by { |user| user.matches.where(season: 3).size }.reverse
